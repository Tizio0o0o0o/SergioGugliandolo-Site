    ...
    if 'help' in command:
                ...
                with open('logfile.txt', 'r') as file:
                    ...
                    for i, line in enumerate(lines):
                        if str(chat_id) in line:
                            del lines[i]
                            break
                lines.append('Chat_id=' + str(chat_id) + ', Numbers=' + str(num) + ', Row=' + str(rw) + ', Turns=' + str(
                    tur) + ', Repetition=' + str(rep) + ', Guess remaining=' + str(tur) + ', Combination=' + str(combination) + ';\n')
                with open('logfile.txt', 'w') as file:
                    file.writelines(lines)

                bot.sendMessage(chat_id, str(
                    'Ok, let\'s start a new game, good lack. I already generated my secret code'))
                break
        if not found:
            ...
            bot.sendMessage(chat_id, str(
                'Settings:\nNumbers=0\nCode length=0\nTurns=0\nRepetition=0'))
            bot.sendMessage(chat_id, str(
                'otherwise substitute the numers you prefer. You can change this settings every time you want, by sending same command'))
        ...
        for i, line in enumerate(lines):
            if str(chat_id) in line:
                ...
                if len(numbers[0]) < 6:
                    bot.sendMessage(chat_id, str(
                        'What solution are you talking about? You firt need to starte a new game, write \"play\" to start'))
                    return
                rw = int(numbers[0][2])
        ...
        if len(res[0]) != 4:
            ...
            bot.sendMessage(chat_id, str(
                'Settings:\nNumbers=0\nCode length=0\nTurns=0\nRepetition=0'))
            bot.sendMessage(chat_id, str(
                'otherwise substitute the numers you prefer'))
            return
        number = int(res[0][0])
        row = int(res[0][1])
        turns = int(res[0][2])
        ...
        if number == 0:
            ...
            bot.sendMessage(chat_id, str(
                'Sorry, \"Numbers\" value needs to be from 5 to 10 (or 0 to use default settings of 6)'))
            return
        if row == 0:
            row = 4
        elif row < 3 or row > 6:
            bot.sendMessage(chat_id, str(
                'Sorry, \"Code length\" value needs to be from 3 to 6 (or 0 to use default settings of 4)'))
            return
        if turns == 0:
            turns = 9
        elif turns < 8 or turns > 12:
            bot.sendMessage(chat_id, str(
                'Sorry, \"Turns\" value needs to be from 8 to 12 (or 0 to use default settings of 9)'))
            return
        if repetition != 0 and repetition != 1:
            bot.sendMessage(chat_id, str(
                'Sorry, \"Repetition\" value needs to be from 0 or 1 (0 for NOT allowed and 1 for allowed)'))
            return
        ...
        for i, line in enumerate(lines):
            if str(chat_id) in line:
                del lines[i]
                break
        lines.append('Chat_id=' + str(chat_id) + ', Numbers=' + str(number) + ', Row=' +
                     str(row) + ', Turns=' + str(turns) + ', Repetition=' + str(repetition) + ';\n')
        with open('logfile.txt', 'w') as file:
            file.writelines(lines)
        ...
        if number == 6 and row == 4:
            ...
            bot.sendMessage(chat_id, str(
                'This version is called \'Beagels\' (1972) and \'Royale Mastermind\' (1972)  and \'Electronic Mastermind\' (1977) and \'Super-Sonic Electronic Mastermind\' (1979)'))
        elif number == 6 and row == 5:
            bot.sendMessage(chat_id, str(
                'This version is called \'Mastermind44\' (1972)'))
        elif number == 10 and row == 4:
            bot.sendMessage(chat_id, str(
                'This version is called \'Grand Mastermind\' (1974) and \'Electronic Mastermind\' (1977) and \'Super-Sonic Electronic Mastermind\' (1979)'))
        elif number == 8 and row == 5:
            bot.sendMessage(chat_id, str(
                'This version is called \'Super Mastermind\' (1972) and \'Deluxe Mastermind\' (1972) and \'Advanced Mastermind\' (1972) and \'Mastermind Challenge\' (1993)'))
        elif number == 10 and row == 5:
            bot.sendMessage(chat_id, str(
                'This version is called \'Electronic Mastermind\' (1977) and \'Super-Sonic Electronic Mastermind\' (1979)'))
        elif number == 10 and row == 6:
            bot.sendMessage(chat_id, str(
                'This version is called \'Super-Sonic Electronic Mastermind\' (1979)'))
        elif number == 5 and row == 3:
            bot.sendMessage(chat_id, str(
                'This version is called \'Walt Disney Mastermind\' (1978'))
        elif number == 8 and row == 4:
            bot.sendMessage(chat_id, str(
                'This version is called \'Parker Mastermind\' (1993) and \'New Mastermind\' (2004)'))
        elif number == 6 and row == 3:
            bot.sendMessage(chat_id, str(
                'This version is called \'Mastermind for Kids\' (1996)'))
        else:
            bot.sendMessage(chat_id, str(
                'That\'s a new version of mastermind, maybe you are the inventor, who knows'))
        ...
        for i, line in enumerate(lines):
            if str(chat_id) in line:
                ...
                if len(numbers[0]) < 6:
                    bot.sendMessage(chat_id, str(
                        'You firt need to starte a new game, write \"play\" to start or \"settings\" to change setings'))
                    return
                num = int(numbers[0][1])
                tur = int(numbers[0][3])
                rw = int(numbers[0][2])
        ...
        if len(pla[0]) != rw:
            bot.sendMessage(chat_id, str('You need to provide ' +
                            str(rw) + ' numbers, not ' + str(len(pla[0])) + '!'))
            return
        ...
        if corr != rw:
            bot.sendMessage(chat_id, str('Correct in correct position: ' +
                            str(corr) + '\nCorrect in wrong position: ' + str(pos)))
            lines.append('Chat_id=' + str(chat_id) + ', Numbers=' + str(num) + ', Row=' + str(rw) + ', Turns=' + str(tur) +
                         ', Repetition=' + str(rep) + ', Guess remaining=' + str(guesstur) + ', Combination=' + str(comb) + ';\n')
            with open('logfile.txt', 'w') as file:
                file.writelines(lines)
            if guesstur == 0:
                bot.sendMessage(chat_id, str(
                    'You finished your turns, I won xD my code was unbreakable'))
                lines.append('Chat_id=' + str(chat_id) + ', Numbers=' + str(num) + ', Row=' +
                             str(rw) + ', Turns=' + str(tur) + ', Repetition=' + str(rep) + ';\n')
                with open('logfile.txt', 'w') as file:
                    file.writelines(lines)
        else:
            bot.sendMessage(chat_id, str(
                'Congratulation, you cracked my code. WELL DONE!'))
            lines.append('Chat_id=' + str(chat_id) + ', Numbers=' + str(num) + ', Row=' +
                         str(rw) + ', Turns=' + str(tur) + ', Repetition=' + str(rep) + ';\n')
            with open('logfile.txt', 'w') as file:
                file.writelines(lines)
